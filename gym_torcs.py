...
class TorcsEnv:
    ...
    def reset(self, relaunch=False):
        ...
        # Modify here if you use multiple tracks in the environment
        self.client = snakeoil3.Client(vision=self.vision)  # Open new UDP in vtorcs
        self.client.MAX_STEPS = np.inf
    ...
    def agent_to_torcs(self, u):
        speed = u[1]
        accel = speed if speed >= 0 else 0
        brake = -speed if speed < 0 else 0

        torcs_action = {'steer': u[0]}

        if self.throttle is True:  # throttle action is enabled
            torcs_action.update({'accel': accel})
            torcs_action.update({'brake': brake})
